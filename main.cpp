...
int main() {
    ...
    // ДЕМОНСТРАЦИЯ НОВОЙ ФУНКЦИОНАЛЬНОСТИ: КНИГИ АВТОРОВ
    std::cout << "\n=== Книги каждого автора (новая функциональность) ===" << std::endl;
    for (const auto& author : library.getAllAuthors()) {
        if (author->hasBooks()) {
            author->printAllBooks();
        }
    }

    // Продемонстрируем получение книг конкретного автора
    std::cout << "\n=== Получение книг автора через getBooks() ===" << std::endl;
    std::cout << "Книги автора " << author6->getName() << ":" << std::endl;
    auto booksOfAuthor6 = author6->getBooks();
    for (const auto& book : booksOfAuthor6) {
        std::cout << "  - " << book->getTitle() << std::endl;
    }

    std::cout << "\n=== Книги соавторов (демонстрация связей) ===" << std::endl;
    auto authorsOfBook3 = library.findBooksByTitle("C++ для начинающих")[0]->getAuthors();
    for (const auto& author : authorsOfBook3) {
        std::cout << author->getName() << " написал " << author->getBooks().size() << " книг(у)" << std::endl;
    }
